...
const ShortcutCard = ({ title, shortcut, description, category }) => {
  ...
  // Parsear el shortcut en un array de teclas
  const parseShortcut = (shortcutStr) => {
    // Primero, normalizamos el formato reemplazando espacios alrededor de los +
    const normalized = shortcutStr.replace(/\s*\+\s*/g, '+');
    // Luego dividimos por + pero preservamos el + cuando es una tecla
    const keys = [];
    let buffer = '';
    
    for (let i = 0; i < normalized.length; i++) {
      if (normalized[i] === '+') {
        // Si hay algo en el buffer, lo agregamos a las teclas
        if (buffer) {
          keys.push(buffer);
          buffer = '';
        }
        // Si el siguiente carácter es '+', entonces este + es una tecla
        if (normalized[i + 1] === '+') {
          keys.push('+');
    ...
    // Agregar cualquier tecla restante en el buffer
    if (buffer) {
      keys.push(buffer);
    }
  ...
  const getGifPath = () => {
    ...
    return `/public/${app}/${level}/${mappedKeys.join(' + ')}.gif`;
  };
    ...
    <div className="bg-white dark:bg-gray-800/90 backdrop-blur-sm rounded-lg border border-gray-100 dark:border-gray-700 p-4">
      ...
      {showGif && (
        <div className="mt-4 rounded-lg overflow-hidden bg-gray-50 dark:bg-gray-900">
          <img
            src={imageError ? '/public/no_image.png' : getGifPath()}
